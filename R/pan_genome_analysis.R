...
PIRATE.gene_families <- as.data.frame(fread("results/PIRATE.gene_families.ordered.tsv")) 
...
singletons_only_by_strain$clade <- name_map$clad[match(row.names(singletons_only_by_strain), name_map$strain)]
accessory_by_strain$clade <- name_map$clad[match(row.names(accessory_by_strain), name_map$strain)]
...
#remove core
all_accessory_anno<- gene_fam_by_strain_w_anno_ones_num2[rowSums(gene_fam_by_strain_w_anno_ones_num2[,5:ncol(gene_fam_by_strain_w_anno_ones_num2)]) < 88,]
...
clade3_names<- accessory_by_strain_list[accessory_by_strain_list == "other"]
...
#there are 279 accessory gene fams exclusive to clade 1
length(clade1_names) #there are 11 strains in clade 1
...
aveclade1_lost #180.2727 to be exact
...
dim(all_totals_lost_in_clade1) #1983 of these
